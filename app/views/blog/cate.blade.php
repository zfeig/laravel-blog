         ...
         <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-6">
            <div class="container">
                 <ul class="nav navbar-nav">
                             <li><a href="/blog/{{$user->username}}">首页</a></li>
                             @foreach($cates as $v)
                             <li @if($v->name==$curCate) class="active" @endif ><a href="/blog/cate/{{$v->id}}">{{$v->name}}</a></li>
                             @endforeach
                           </ul>
            </div>
...
<!--左边部分-->
                     <div class="left">
                      <!--文章列表-->
                         <div class="list">
                           @if($count==0)
                           <h3><span class="notice">{{$curCate}}</span>栏目下没有留下任何痕迹!</h3>
                           @else
                                 <!--面包屑导航-->
                                    <ol class="breadcrumb">
                                      <li><a href="/blog/{{$user->username}}">博客首页</a></li>
                                      <li><a href="{{Request::url()}}">{{$curCate}}</a></li>
                                    </ol>
